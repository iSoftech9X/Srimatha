...
const OrderManagement: React.FC = () => {
  ...
  useEffect(() => {
    const token = localStorage.getItem('token');
    const newEventSource = new EventSource(`http://localhost:5000/api/orders/events?token=${token}`);
    ...
    newEventSource.addEventListener('new_order', (event) => {
      try {
        const data = JSON.parse(event.data);
    ...
    newEventSource.addEventListener('order_update', (event) => {
      try {
        const data = JSON.parse(event.data);
        setRealTimeOrders(prev => 
          prev.map(order => 
    ...
    newEventSource.addEventListener('new_quote', (event) => {
      try {
        const data = JSON.parse(event.data);
        setRealTimeQuotes(prev => [data, ...prev]);
          ...
          <div className="bg-white rounded-lg shadow-lg max-w-lg w-full z-10">
            <div className="p-6">
              ...
              <div className="space-y-4">
                ...
                <div>
                  <span className="text-sm text-gray-500">Customer:</span>
                  <span className="text-gray-800 font-semibold"> {selectedOrder.customer?.name || 'N/A'}</span>
                </div>
                <div>
                  <span className="text-sm text-gray-500">Phone:</span>
                  <span className="text-gray-800 font-semibold"> {selectedOrder.customer?.phone || 'N/A'}</span>
                </div>
                <div>
                  <span className="text-sm text-gray-500">Order Date:</span>
                  <span className="text-gray-800 font-semibold"> {new Date(selectedOrder.created_at).toLocaleString()}</span>
                </div>
                <div>
                  <span className="text-sm text-gray-500">Status:</span>
                  <span className={`inline-flex items-center px-2 py-1 text-xs font-semibold rounded-full border ${getStatusColor(selectedOrder.status)}`}>
