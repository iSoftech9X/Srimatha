...
const MenuManagement: React.FC = () => {
  ...
  const [formData, setFormData] = useState({
    ...
    isVegan: false,
    isGlutenFree: false,
    spiceLevel: '', // Removed default 'mild' value
    preparationTime: '',
    available: true,
    popular: false
  });
  ...
  const spiceLevels = [
    { value: '', label: 'Select Spice Level' }, // Added empty option
    { value: 'none', label: 'No Spice' },
    { value: 'mild', label: 'Mild' },
    { value: 'medium', label: 'Medium' },
    { value: 'hot', label: 'Hot' },
  ...
  const handleEdit = (item: MenuItem) => {
    ...
    setFormData({
      ...
      isVegan: item.isVegan || false,
      isGlutenFree: item.isGlutenFree || false,
      spiceLevel: item.spiceLevel || '', // Use empty string if no spice level from API
      preparationTime: item.preparationTime?.toString() || '',
      available: item.available,
            ...
            <div key={item.id} className="bg-white rounded-lg shadow-md overflow-hidden hover:shadow-lg transition-shadow duration-300">
              <div className="p-4">
                ...
                <div className="mb-3">
                  ...
                  {item.spiceLevel && (
                    <span className="inline-block bg-red-100 text-red-800 px-2 py-1 rounded-full text-xs font-medium ml-2 capitalize">
                      {item.spiceLevel} {item.spiceLevel !== 'none' ? 'Spice' : ''}
                    </span>
                  )}
                </div>
