...
import React, { useState, useEffect } from 'react';
import { Star, Plus, ShoppingCart, Users, Calendar, X, History, Minus, Search } from 'lucide-react';
import { useAuth } from '../context/AuthContext';
import { useApp } from '../context/AppContext';
import { menuAPI, ordersAPI, authAPI } from '../services/api';
import toast from 'react-hot-toast';
...
const CateringOrdering: React.FC = () => {
  ...
  const groupedItems = filteredItems.reduce((acc, item) => {
    const category = menuCategories.find(c => c.id === item.category) || { id: 'other', name: 'Other' };
    if (!acc[category.id]) {
      acc[category.id] = {
            ...
            {loading ? (
              ...
              <div className="space-y-12 mb-12">
                {Object.entries(groupedItems).map(([categoryId, { categoryName, items }]) => (
                  <div key={categoryId} className="bg-white rounded-xl shadow-md overflow-hidden">
                    <div className="p-6 border-b border-gray-200">
                      <h2 className="text-xl font-bold " style={{color:"#501608"}}>{categoryName}</h2>
                    </div>
